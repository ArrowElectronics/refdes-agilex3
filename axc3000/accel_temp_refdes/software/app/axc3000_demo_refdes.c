/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * axe3000_demo_refdes.c is a demo program running on the AXC3000 eval board.
...
#define U10_addr    0x0E	// FS1606 Power Module
...
void dispTemp()
{
	...
	if(ret_code == 0)
	{
		printf("Agilex-3C Location 0 Sensor 1 Temp. = %ld.%02dC\r\n",(resp_buf[0]>>8) & 0x0FF, fracT(resp_buf[0]&0x0FF));
	}
   	...
   	if(ret_code == 0)
   	{
   		printf("Agilex-3C Location 1 Highest  Temp. = %ld.%02dC\r\n",(resp_buf[0]>>8) & 0x0FF, fracT(resp_buf[0]&0x0FF));
   	}
   	...
   	if(ret_code == 0)
   	{
   		printf("Agilex-3C Location 3 Highest  Temp. = %ld.%02dC\r\n",(resp_buf[0]>>8) & 0x0FF, fracT(resp_buf[0]&0x0FF));
   	}
   	...
   	if(ret_code == 0)
   	{
   		printf("Agilex-3C Location 4 Highest  Temp. = %ld.%02dC\r\n",(resp_buf[0]>>8) & 0x0FF, fracT(resp_buf[0]&0x0FF));
   	}
